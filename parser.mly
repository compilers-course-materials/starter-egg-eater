...
%token <int> NUM
%token <string> ID
%token END LBRACK RBRACK DEF ADD1 SUB1 LPAREN RPAREN LET IN EQUAL COMMA PLUS MINUS TIMES IF COLON ELSECOLON TRUE FALSE ISBOOL ISNUM ISTUPLE EQEQ LESS GREATER PRINT EOF
...
simple_expr :
  ...
  | LPAREN tuple_exprs RPAREN { ETuple($2) }
  | LPAREN expr RPAREN { $2 }
  | ID LPAREN exprs RPAREN { EApp($1, $3) }
  | ID LPAREN RPAREN { EApp($1, []) }
  | simple_expr LBRACK binop_expr RBRACK { EGetItem($1, $3) }
  | const { $1 }
  | ID { EId($1) }
